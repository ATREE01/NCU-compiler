...
{
  "\"end of file\"", "error", "\"invalid token\"", "END", "NUMBER",
  "$accept", "startsymbol", "numbers", "number", YY_NULLPTR
};
...
yyreduce:
  ...
  switch (yyn)
    {
  case 2: /* startsymbol: numbers END  */
#line 51 "B.y"
                         {
        if(!isEmpty())
...
#line 1098 "B.tab.c"
  ...
  case 3: /* numbers: numbers number  */
#line 58 "B.y"
                         {
        while(true){
            // printf("Number: %d, top(): %d\n", $2, top());
            if((yyvsp[0].ival) == train){
                printf("Train %d passing through\n", train++);
                break;
            }
            else if(!isEmpty() && (top() == (yyvsp[0].ival))){
                pop();
                break;
            }

            else if(isEmpty() && (yyvsp[0].ival) < train){
...
#line 85 "B.y"
        ...
        while(true){
            if((yyvsp[0].ival) == train){
                ...
                if(top() > (yyvsp[0].ival)){
                    printf("Error: Impossible to rearrange\n");
                    printf("The first train in the holding track is train %d instead of train %d", top(), (yyvsp[0].ival));
                    yyerror("");
                }
                push(train++);
            }
